...
export async function POST(request: NextRequest) {
  try {
    const { prompt, style, userName, userAge, userGender } = await request.json()
    
    console.log("원본 프롬프트:", prompt)
    console.log("스타일:", style)
    console.log("사용자 정보:", { userName, userAge, userGender })
    
    // 새로운 스토리 기반 프롬프트 구조 처리
    let cleanPrompt = "";
    ...
    if (scene) {
      ...
      if (translatedKeywords.length > 0) {
        storyPrompt = `A beautiful ${selectedStyle} illustration featuring: ${translatedKeywords.join(', ')}. ${mood} atmosphere, warm and nostalgic feeling, no text, no Japanese style, no Chinese style, no American style, only culturally neutral elements. High quality, detailed artwork.`;
      } else {
        storyPrompt = `A beautiful ${selectedStyle} illustration with ${mood} atmosphere, warm and nostalgic feeling, no text, no Japanese style, no Chinese style, no American style, only culturally neutral elements. High quality, detailed artwork.`;
      }
    } else {
      // 기본 프롬프트
      storyPrompt = `A beautiful ${selectedStyle} illustration with ${mood} atmosphere, warm and nostalgic feeling, no text, no Japanese style, no Chinese style, no American style, only culturally neutral elements. High quality, detailed artwork.`;
    ...
    if (!openaiApiKey) {
      ...
      return NextResponse.json({
        imageUrl: `/placeholder.svg?height=400&width=400&text=No+API+Key+${Math.floor(Math.random() * 1000)}`,
        success: true,
      })
    }
    ...
    return NextResponse.json({
      imageUrl: `/placeholder.svg?height=400&width=400&text=API+Failed+${Math.floor(Math.random() * 1000)}`,
      success: true,
      model: "placeholder"
    })
  } catch (error) {
