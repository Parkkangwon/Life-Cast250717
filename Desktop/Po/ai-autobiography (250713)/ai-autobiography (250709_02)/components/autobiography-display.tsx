...
export default function AutobiographyDisplay({ sections, images, onBack, imageStyle }: AutobiographyDisplayProps) {
  ...
  const expandAnswer = (question: string, answer: string, sectionId: string) => {
    ...
    const expansions = {
      ...
      present: {
        prefix: "현재를 살아가며,",
        suffix: "이 모든 것들이 지금 이 순간을 더욱 의미 있게 만들어준다.",
      },
      future: {
        prefix: "미래를 그려보며,",
        suffix: "이 꿈들이 내일을 향한 희망의 등불이 되어줄 것이다.",
      },
    }

    const expansion = expansions[sectionId as keyof typeof expansions]
    return expansion ? `${expansion.prefix} ${answer} ${expansion.suffix}` : answer
  }
  ...
  const getBackgroundImage = (sectionId: string) => {
    const backgrounds = {
      ...
      present: "/images/frame-elegant.png",
      future: "/images/frame-elegant.png",
    }
